...
struct Arguments
{
    ...
    std::string markDeviceName = "unknowDevice";
    bool mark = false;
...
static CommandLineParser<Arguments> buildCmdLineParser()
{
    ...
    std::vector<const CommandLineOption<Arguments>*> options {
        ...
        new ArgumentOption<Arguments>(
            [] (Arguments &state, const std::string &name) { state.markDeviceName = name; state.mark = true; },
...
int main(int, const char * const *argv)
{
    ...
    if(args.mark){
        ...
        oss << std::put_time(&tm, "mark_%Y%m%d_%H%M%S_") << args.markDeviceName << ".csv";
        ...
        outputFile << "# GPU Model: " << args.markDeviceName << "\n";
        ...
        for(int mcost =min_difficulty; mcost <= max_difficulty; mcost+=step){
            ...
            printf("mark difficulty:%d, batchSize:%d\n", mcost, batchSize);
        }
        outputFile.close();
        return 0;
    }
    std::ifstream file("diff.txt");
    if (file.is_open()) {
        int new_difficulty;
        if (file >> new_difficulty) { // read difficulty
            std::lock_guard<std::mutex> lock(mtx);
    ...
    std::thread t(read_difficulty_periodically, "diff.txt"); 
